...
function bgwhiteone()
{
	if (window.confirm('Поменять цвет фона?')==true)
		{
	bgwhite();
	window.alert('Обновите страницу для возвращения к стандартному фону, либо поменяйте цвет еще раз и выбирите "Отмена"');
		}
	else
	//alert('Нажмите "Отмена" для возвращения к стандартному фону');
	bgblack();
}
